...
export const createInitialTestProduct = (userId) => {
  ...
  fireDb
    .ref("products/" + userId + `/product_1`)
    .set(productData)
    .then(() => {
      console.log("succefully written initial data");
    })
...
export const addProduct = (productData, userId) => {
  ...
  fireDb
    ...
    .then((snapshot) => {
      ...
      fireDb
        .ref("products/" + userId + `/product_${productCount + 1}`)
        .set(productData)
        .then(() => (dispatch) => {
          fireDb.ref("products/" + userId).on("value", (data) => {
...
export const deleteProductFromDatabase = (productId, userId) => {
  ...
  fireDb.ref(`products/${userId}/${productId}`).remove(() => {
    console.log("deleted");
  });
};
